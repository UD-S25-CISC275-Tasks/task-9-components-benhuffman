...
export function isValid(question: Question, answer: string): boolean {
    ...
    if (question.type === "multiple_choice_question") {
        //must be exactly one of the options and no others
...
export function toMarkdown(question: Question): string {
    var output = `# ${question.name}\n${question.body}`;
    if (question.type === "multiple_choice_question") {
        output +=
            "\n" +
            question.options.map((option: string) => `- ${option}`).join("\n");
        //output += ["response one", "response two", response three]
    }

    //  console.log(output);
    return output;
    //so .map() DOES NOT MUTATE, it returns a new array
}
...
export function mergeQuestion(
    ...
    return { ...contentQuestion, published: false, points, id: id, name: name };
}